...
| Gate 3 (Corner Finder PWA + demo mode) | **Field test passed (Alton, Sep 2026)** — live-GPS walk worked well; HUD constants stay at defaults unless later walks say otherwise. `docs/Gate-3-Phase-Report.md` |
...
| Gate 6 (assemble automation, hardening, Lighthouse) | **In progress (approved Sep 23)** — shipped: Assemble flow (KML at creation, assembly checklist, Generate remaining), rate limits + security review, admin Activity trail, first-sign-in welcome, Lighthouse perf+a11y gate in CI (`lighthouse-budget.json`; perf ≥ 0.80, a11y ≥ 0.95 on the walk page, emulated mobile). Remaining: ops docs (runbook, team guide), Gate-6 acceptance run (second property < 1 hr). Disclaimer attorney-approved; Vercel/Supabase Pro live. |
...
- **Auth email**: Gmail Workspace SMTP (`smtp.gmail.com:587`, app password, sender/username `info@texasgreenerpastures.com`). Email templates for **Magic Link** and **Confirm signup** are customized to `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=email` (cross-device safe). `/auth/confirm` also accepts the PKCE `?code=` shape. **Template copy matters**: a new invitee gets the *Confirm signup* template, and stock Supabase wording ("Confirm your signup") reads like something they never requested — both templates should carry the sign-in-link copy (recommended body is in the Gate-6 notes / chat log of Sep 23: subject "Your Greenways sign-in link", body explaining the link *is* the sign-in, no password, ignore if unexpected). First-ever sign-in also lands on `/admin/properties?welcome=1` with an in-app explainer, server-detected via `first_signed_in_at`.
...
13. **Walk terms / liability disclaimer**: the welcome screen's Start button carries "By starting, you agree to the walk terms" with the full bilingual text one tap away (`TermsSheet`, `terms.*` in both catalogs). Tapping Start logs `disclaimer_acknowledged`, flushed to `walk_events` immediately (not batched) — the timestamped per-session receipt. Wording attorney-approved Sep 23 (changes to it go back through Alton).
14. **Admin activity trail** (`/admin/activity`, admin-only nav + RLS via migration `20260923210000`): every guarded action already wrote to `audit_log` (locks/unlocks, publishes, media review, uploads, API keys, Monday sync); sign-ins now do too (`signed_in` via `/auth/confirm`, best-effort). The page shows last sign-in per team member and the latest 200 entries with plain-language labels (`ACTION_LABELS` in the page — add a label when adding a `write_audit` action). Reads are admin-only; rows are append-only (no update/delete path anywhere).
15. **Assembly pipeline** (`lib/assembly.ts`): the Overview checklist derives every stage (geometry → verify → photos → brief → style → batch → spanish → publish) purely from data — change what "done" means there, not in the page. The New-property dialog takes an optional KML (parse-before-insert; a bad file never leaves a bare row), and the Media tab's "Generate remaining" batches every ready slot through the same guarded `queueSlot` path once the style lock is open.
...
16. **Master/lot hierarchy + subdivision splitter** (migration `20260924100000`, `lib/kml.ts` `parseKmlMulti`, `lib/geo/subdivision.ts`): `properties.parent_id`, one level deep, enforced by the `property_parent_guard` trigger (self-parent, nesting, and master-with-lots-becoming-a-lot all raise; deleting a master orphans its lots to top level on purpose). A multi-polygon KML at creation — or "Split master KML" on a standalone property's Overview — creates one lot property per polygon through the normal `applyParsedKml` corner path; the master holds no geometry, its Overview shows the Lots table instead of the assembly checklist, and the source KML is filed under the master's Documents. Phantom-vertex pruning uses BOTH guards in `pruneCollinear`: deviation < 10 ft **and** sliver triangle < 2,000 sqft — the area guard exists because Warren lot 9 has a real corner 6.9 ft off the line next to a true phantom 0.0 ft off, and deviation alone pruned the real corner first (cost: 0.08 acres). Fixture `data/warren_land_plan.kml` (10 lots, 9×~4.98 ac + 7.83 ac); folder recursion in `collectPlacemarks` is required — Google Earth Pro nests Document → Folder → Folder → Placemark and the old one-level parser threw "KML has no Placemark".

17. **Never export data from a `"use client"` module to server components.** When a server component imports from a client module, every export becomes a client reference, and property lookups on a referenced object silently return `undefined` — no error. This shipped sale pills as `pill-undefined` (right text, no color) on the Properties list and master Lots table, and cost six CI rounds on PR #27 to isolate because everything else (DB write, revalidate, repaint) was actually working. Shared constants/maps live in server-safe modules (`components/ui/pill.tsx` holds `statusTone` + `saleTone`); client components import from there, never the reverse. Related: the E2E job now prints Playwright's `error-context.md` page snapshots straight into the CI log (`.github/workflows/ci.yml`) — read those before theorizing, they show exactly what the page rendered at failure. On the master page, "Edit all lots" (`updateAllLots`) reloads on success by design.
...
1. ~~Gate 3 field test~~ **Passed** (live GPS walk worked well). The `hillmont-gps-test` square remains for future tuning; HUD constants (`lib/hud/arrival.ts`, `boundary.ts`, `filters.ts`) stay at defaults.
2. ~~Team roster activation~~ **Alton is inviting the team himself** from the admin Team tab — nothing to build.
...
5. Deferred small items: `language_switched` event on the toggle; per-property declination (constant +1.5°E now); Mapbox satellite variant of the buyer mini-map (Google variant shipped; drawn = offline fallback); ~~Lighthouse-in-CI~~ (done: perf/a11y/budget gate in the E2E job; PWA category is gone from v12, offline stays covered by Playwright); walk access tokens (Phase 5) — until then any slug with locked corners serves.
