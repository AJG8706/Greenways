...
14. **Assembly pipeline** (`lib/assembly.ts`): the Overview checklist derives every stage (geometry → verify → photos → brief → style → batch → spanish → publish) purely from data — change what "done" means there, not in the page. The New-property dialog takes an optional KML (parse-before-insert; a bad file never leaves a bare row), and the Media tab's "Generate remaining" batches every ready slot through the same guarded `queueSlot` path once the style lock is open.
